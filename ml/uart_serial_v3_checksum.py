        ...
        while self.read_sensor_flag:
            ...
            while(count < 145):
                ...
                if (len(received_string)>10):
                    ...
                    # row = pd.read_csv(io.BytesIO(self.header+received_string), sep=',' )
                    ...
                    if cv == True:
                        ...
                        row = pd.read_csv(io.BytesIO(self.header+rs), sep=',' )
                    # row = pd.read_csv(io.BytesIO(header+received_string.rsplit(',', 1)[0]), sep=',' )
                    # if row.isnull().values.any() == False:
                        # if(row.ax0[0] == -1 and row.ay0[0] == -1 and row.az0[0] == -1 and row.gx0[0] == -1 and row.gy0[0] == -1 and row.gz0[0] == -1 ):
                        #     raise ValueError("Sensor 0 down, I repeat sensor 0 is down")
                        # if(row.ax1[0] == -1 and row.ay1[0] == -1 and row.az1[0] == -1 and row.gx1[0] == -1 and row.gy1[0] == -1 and row.gz1[0] == -1 ):
                        #     raise ValueError("Sensor 1 down, I repeat sensor 1 is down")
